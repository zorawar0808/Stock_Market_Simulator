...
async def _run(action_name: str, fn, admin: Admin, db: AsyncSession):
    try:
        state = await fn(db)
    ...
    await audit_record(
        db, actor=f"admin:{admin.admin_id}", action=action_name.upper(),
...
async def start(admin: Admin = Depends(require_role(*_CONTROL_ADMIN_ROLES)), db: AsyncSession = Depends(get_db)):
    return await _run("market start", market_service.start_market, admin, db)
...
async def resume(admin: Admin = Depends(require_role(*_CONTROL_ADMIN_ROLES)), db: AsyncSession = Depends(get_db)):
    return await _run("market resume", market_service.resume_market, admin, db)
...
async def unfreeze(
    ...
    return await _run("unfreeze", market_service.unfreeze_to_live, admin, db)
